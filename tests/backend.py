...
with eel.import_frontend_functions():
    ...
    # import web.frontendscrypt
    from web.frontendscrypt import show_previous_results, logdone

#from web.frontendscrypt import SharedState
...
if __name__ == "__main__":
    ...
    if DEV:
        sys.argv = ["transcrypt.py", "-e", "6", "-b", "./web/fronts.py", "-n"]
        sys.argv[0] = re.sub(r"(-script\.pyw?|\.exe)?$", "", sys.argv[0])
        transpile()
        #sys.argv = ["transcrypt.py", "-e", "6", "-b", "./web/anotherfrontend.py", "-n"]
        #sys.argv[0] = re.sub(r"(-script\.pyw?|\.exe)?$", "", sys.argv[0])
        #transpile()
